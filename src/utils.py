...
def get_chracter_creation_date(chr_name, server, server_name2id, api_key_val):
    try:
        chr_url = api_url + 'servers/' + server_name2id[server]+'/characters?characterName=' + \
            urllib.parse.quote(chr_name) + '&apikey=' + api_key_val
        ...
        while not stop_idx :
            ...
            timeline_url = api_url + 'servers/'+ server_name2id[server]+'/characters/' + \
                chr_info['characterId'] + '/timeline?code=101&startDate='+past_str+ '&endDate='+ current_str+ '&apikey=' + api_key_val
