...
for i in range(len(base_job_list)):
    time.sleep(1)
    job_page = driver.find_element(By.XPATH, '/html/body/section/div[1]/div[1]/div/div[2]/div[2]/ul['+str(i//5 +1)+']/div['+str(i%5 +1)+']')
    job_page.click()
    page_src = BeautifulSoup(driver.page_source)
    job_dict[base_job_list[i]] = [re_job.sub( '',x.get_text()) for x in page_src.find('div', class_ = 'ch_evol tble').find_all('div', class_ = 'rkcbox') if 'none' not in x['style']]

        
job_url= 'https://dundam.xyz/damage_ranking?page=1&type=7&job='+job+ '&baseJob='+base_job+'&weaponType=%EC%A0%84%EC%B2%B4&weaponDetail=%EC%A0%84%EC%B2%B4'
...
while not stop_idx:
    for j in range(1, 11): 
        ...
        ## 다음페이지에 해당하는 버튼 누르기 
        next_page = driver.find_element_by_xpath('//*[@id="ranking"]/div[1]/div[5]/div[3]/div/ul/li['+ str(2+j) +']')
        next_page.click()
        
        if stop_idx:
            break
    #중간 저장 
    with open('C:/Neople_Anal/DAT/'+base_job+'_'+job+'_info', "wb") as fp: 
